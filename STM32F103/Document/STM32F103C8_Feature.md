...
SysTick 定时器也叫 SysTick 滴答定时器，是 Cortex-M3 内核嵌入到 NVIC 中的定时器

- 24 位的向下递减定时器
- SYSTICK 的时钟是 AHB 时钟 8 分频后提供，最大周期 = 1/(72/8) = 1 / 9 us
  - 1us 需要计数 9 次
...
- 基本定时器： 只有定时功能
- 通用定时器： 在基本定时器上扩展了输入捕获和输出比较
  - 输入捕获：测量输入信号脉冲宽度
  - 输出比较：PWM 或 输出波形比较
- 高级定时器： 在通用定时器上扩展了可编程死去互补输出，带刹车功能等

定时器的功能比较多，有什么新的功能可以尝试使用定时器
...
CK_INT 是内部时钟，是APB2、APB1 的时钟

- TIM1 是高级定时器，挂接在 APB2 上
- TIM4 是通用定时器，挂接在 APB1 上
- 定时器时钟，分频系数 等于1，时钟是 APB1 时钟，不等于 1，时钟是 APB1 的 2倍
  - APB1 的时钟是 36MHZ，如果时钟分频数不为 1，则 APB1 时钟是 72MHZ
- TIM4 的预分频器可以实时更改，在下一更新事件发生时被采用
- TIM4 计数方式可以设置成：向上计数，向下计数，向上向下计数
- 输入捕获，可以对信号的上升沿，下降沿，双边沿进行捕获
  - 常用于测量输入信号脉宽，PWM 输入信号的频率及占空比
  - 输入通道，捕获通道是不同的，一个输入通道的信号可以同时输入给两个捕获通道
- 输出比较：计数器和设定值相等，则输出信号极性可变
  - 可以输出，有效电平，无效电平，反转等
  - 使用最多的是 PWM1 和 PWM2
...
- 使能定时器时钟
- 初始化定时器参数，时钟，计数方式，重装载值
  - 重装载值应该 -1，因为定时器预分频器内部会自动 +1；
- 设置中断，使能
- 设置 NVIC
- 开启定时器
- 编写中断服务函数
...
- 独立看门狗由 LSI 内部低速时钟驱动，频率在 30KHZ-60KHZ 之间，通常按照 40KHZ 计算
- 12 位递减计数器，减到 0 会产生复位，在减到 0 之前重新装载计数器值，称为清看门狗
- IWDG 的时钟是 40KHZ 经一个 8bit 的分频器分频之后得到，可以分频 1，2，4，256
- 看门狗时间计算公式：溢出时间 Tout = (4*2^pre) / 40 * rlr(ms)，pre 是预分频器系数（0-6），rlr 是重装载寄存器的值，40 是40KHZ
  - pre = 4，rlr = 800； 看门狗溢出时间是 1280ms

配置步骤

- 开启寄存器访问；IWDG 具有写保护，要 IWDG_KR 写入 0x5555 才能访问
  - 被封装到库函数：IWDG_WriteAccessCmd(IWDG_WriteAccess_Enable);
  - 写入其他值会开启写保护，比如 喂狗，和使能 IWDG，写入的是其他值
- 设置分频系数和重载值；
- 重载计数器值（喂狗）； 给 IWDG_KR 中写入 0xAAAA
- 开启 IWDG 给 IWDG_KR 中写入 0xCCCC

注意：

- IWDG 一旦开启，就不能被关闭，想要关闭只有重启，然后不开启 IWDG
- 所有对 IWDG 的操作都封装到库函数中了，直接使用，不需要配置寄存器
