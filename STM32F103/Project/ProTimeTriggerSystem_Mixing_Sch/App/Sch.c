...
//------------------------------------------------------------------------
//- 公用变量定义
//- 数据类型 sTask 和常数 hSCH_MAX_TASKS 创建任务队列，被调度器引用
//------------------------------------------------------------------------
...
*       Co-op：如果任务是合作式的，设置为 1； 如果任务是抢占式的，设置为 0
* Output:
*       返回值：返回任务在队列中的位置，如果小于 hSCH_MAX_TASKS，则添加成功，如果等于
*              SCH_MAX_TASKS，则队列满了
*       注意：删除任务，需要用到这个返回值 hSCH_Delete_Task()
* 例子：
*       Task_ID = hSCH_Add_Task(Do_X, 1000, 0, 0); 使函数 Do_X 在 1000 个调度器时标之后运行一次(抢占式任务)
*       Task_ID = hSCH_Add_Task(Do_X, 0, 1000， 1); 使函数 Do_X 每隔 1000 个调度器时标运行一次（合作式任务）
*       Task_ID = hSCH_Add_Task(Do_X, 300, 1000， 0);
*       使函数 Do_X 每隔 1000 个调度器时标运行一次。且任务首次运行在第 300 个时标，然后1300，2300...（抢占式任务）
*************************************************************************/
...
/*************************************************************************
* Name: hSCH_Delete_Task
* Function: 从调度器删除任务，
* Input: TASK_INDEX 任务索引，由 hSCH_Add_Task() 提供
...
/*************************************************************************
* Name: hSCH_Dispatch_Tasks
* Function: 调度程序，任务需要 hSCH_Dispatch_Tasks 来运行，被 main 循环调用
* Input:
* Output:
*************************************************************************/
void hSCH_Dispatch_Tasks(void)
{
    ...
    for (Index = 0; Index < hSCH_MAX_TASKS; Index++)
    {
        ...
        if ((hSCH_tasks_G[Index].Co_op) && (hSCH_tasks_G[Index].RunMe > 0))
        {
            ...
            {
                // 比通过调用 hSCH_Delete_Task(Index); 来删除任务更快
                hSCH_tasks_G[Index].pTask = 0;
            }
        }
    }
    hSCH_Report_Status();  // 报告系统状况
    hSCH_Go_To_Sleep();  // 这里调度器进行空闲模式
}
            ...
            if (hSCH_tasks_G[Index].pTask)
            {
                if (hSCH_tasks_G[Index].Delay == 0)
                {
                    ...
                    else
                    {
                        // 如果是抢占式任务，立即运行它
                        (*hSCH_tasks_G[Index].pTask)();    // 运行任务
                        hSCH_tasks_G[Index].RunMe -= 1;    // RunMe 标志位，复位/减一
                        // 周期性的任务将再次运行
                        // 单次任务，将从任务队列中删除
                        if (hSCH_tasks_G[Index].Period == 0)
                        {
