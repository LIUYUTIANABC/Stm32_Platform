...
前面创建的 TCB 是没有优先级的，后面需要添加优先级。

- 在 STM32 中的存储器的增长方式是由高到低的，所以，就绪列表优先级排序是倒过来的
  - 即：优先级表的 bit[31] 对应最高优先级 Prio = 0；
  - Prio = 3 对应优先级表中的 bit[28]
  - OSPrioTb1[0] 的位 31 表示是优先级最高的任务；

前导零，后导零

- 前导0：从高位开始找 1 叫计算前导 0
  - CPU_CntLeadZeros();
  - STM32 是小端模式，所以应该用前导0，即：从高位开始找 1
- 后导0：从低位开始找 1 叫计算后导 0
  - CPU_CntTrailZeros();
  - 有些芯片是大端模式，应该计算后导0
- STM32 支持前导零指令 CLZ 可以由汇编来实现，加快指令运算

就绪列表存储的是相同优先级的任务控制块

- 就绪列表数量和优先级表数量一一对应，每一个优先级对应一个就绪列表
  - 区别：就绪列表存储的是双向链表
  - 双向链表，头节点，尾节点，根节点；

#### 支持多优先级

为任务添加优先级，并把任务添加到就绪列表当中，在任务调度中，选择最高优先级的任务运行；

#### 时基列表

处于延时的任务和等待事件有超时限制的任务都会从就绪列表中移除，然后插入时基列表；

- 时基列表在 OSTimeTick 中更新
- 如果任务的延时时间结束或者超时到期，就会让任务就绪，从时基列表移除，插入就绪列表
- 目前为止，任务要么在就绪列表，要么在时基列表

#### 时间片

让 OS 支持同一个优先级下可以有多个任务；

- 运转方式：这些任务可以分配不同的时间片，当任务时间片用完，任务会从链表的头部移动到尾部
  - 让下一个任务共享时间片，以此循环。
- 注意：同一优先级下的任务分配时间片去运作；这些任务可以使用阻塞延时也可以使用软件延时，
  - 但是，延时的时间都必须小于时间片

#### 任务的挂起，恢复和删除

- 挂起：就相当于暂停，暂停后任务从就绪列表中移除
  - 挂起可以挂起自己
- 恢复：即重新将任务插入就绪列表
  - 恢复被挂起的函数，但是不能恢复自己
- 一个任务挂起多少次就要被恢复多少次才能重新运行
- 删除：一个任务被删除后就进入休眠态，要想继续运行必须创新创建
